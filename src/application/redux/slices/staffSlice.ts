...
import { StaffResponseDTO, StaffListResponse } from "@/domain/dto/StaffDTO";
import {
  fetchStaffList,
  fetchStaffById,
  createStaff,
...
const staffSlice = createSlice({
  ...
  reducers: {
    ...
    setCurrentStaff: (
      state,
      action: PayloadAction<StaffResponseDTO | null>
    ) => {
  ...
  extraReducers: (builder) => {
    ...
    builder
      ...
      .addCase(
        ...
        (state, action: PayloadAction<StaffListResponse | null>) => {
          state.loading = false;
          if (action.payload) {
      ...
      .addCase(fetchStaffList.rejected, (state, action) => {
        state.loading = false;
        state.error =
          (action.payload as string) || "Không thể tải danh sách nhân viên";
      });
    ...
    builder
      ...
      .addCase(
        ...
        (state, action: PayloadAction<StaffResponseDTO | null>) => {
          state.loading = false;
          state.currentStaff = action.payload;
        }
      )
      .addCase(fetchStaffById.rejected, (state, action) => {
        state.loading = false;
        state.error =
          (action.payload as string) || "Không thể tải thông tin nhân viên";
      });
    ...
    builder
      ...
      .addCase(updateStaff.rejected, (state, action) => {
        state.loading = false;
        state.error =
          (action.payload as string) || "Không thể cập nhật nhân viên";
      });
